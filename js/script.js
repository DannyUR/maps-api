...
// no olvidar habilitar Maps Service, Places Service (new), Directions API y geolocation services
...
let currentSearch = "Restaurantes, Cafés, Tiendas, Gasolineras, Hoteles";
let allPlaces = [];
...
async function findPlaces(searchText) {
  ...
  try {
    ...
    if (places.length) {
      ...
      restaurantListElement.innerHTML = `<p class='text-center mt-4'>No se encontraron resultados de "${searchText}".</p>`;
    }
  } catch (error) {
    restaurantListElement.innerHTML = `<p class='text-center text-danger mt-4'>Error al buscar.</p>`;
  }
}
  ...
  const card = `
    <div class="restaurant-card p-3" 
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="reviews text-muted small"><i class="fa fa-comment"></i> ${
...
// ✅ NUEVA FUNCIÓN drawDistance CON RUTAS REALES
async function drawDistance(center, places) {
  ...
  // Encuentra lugar más cercano y más lejano
  let nearest = null;
  let farthest = null;
  ...
  // Ruta al lugar más cercano
  if (nearest) {
    const directionsRenderer = new DirectionsRenderer({
      map: map,
      suppressMarkers: false,
      polylineOptions: {
        strokeColor: "#432cedff",
        strokeWeight: 4,
        strokeOpacity: 0.9,
      },
    });
    ...
    directionsService.route(request, (result, status) => {
      ...
      else console.error("Error al trazar la ruta:", status);
    });

    window.directionsRenderer = directionsRenderer;
  }

  // Ruta al lugar más lejano (verde)
  if (farthest) {
    const directionsRenderer2 = new DirectionsRenderer({
      map: map,
      suppressMarkers: false,
      polylineOptions: {
        strokeColor: "#11c311ff",
        strokeWeight: 4,
        strokeOpacity: 0.9,
      },
    });
    ...
    directionsService.route(request2, (result, status) => {
      if (status === "OK") directionsRenderer2.setDirections(result);
      else console.error("Error al trazar la ruta lejana:", status);
    });
  }
}

function filterPlaces(places, center, radius) {
...
async function displayAll(places) {
  restaurantListElement.innerHTML = "";
  markers.forEach((marker) => (marker.map = null));
  markers = [];
...
function placesByRating() {
  allPlaces.sort((a, b) => (b.rating || 0) - (a.rating || 0));
  applyFiltersAndDisplay();
}

function placesByReviews() {
...
async function applyFiltersAndDisplay() {
  ...
  const filteredPlaces = filterPlaces(allPlaces, avgCenter, currentRadius);

  await displayAll(filteredPlaces);

  if (avgCenter) {
    await drawCircle(avgCenter, currentRadius);
    await drawDistance(avgCenter, filteredPlaces);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sortRatingButton) {
    sortRatingButton.addEventListener("click", placesByRating);
  }

  const sortReviewsButton = document.getElementById("sort-reviews");
  if (sortReviewsButton) {
    sortReviewsButton.addEventListener("click", placesByReviews);
  }
